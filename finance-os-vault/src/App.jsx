...
const defaultSources = {
  cash: [
    { id: "c1", label: "DBS Savings" },
    { id: "c2", label: "OCBC 360" },
  ],
  investments: [
    { id: "i1", label: "Syfe" },
    { id: "i2", label: "StashAway" },
    { id: "i3", label: "Tiger Brokers" },
  ],
};
...
const dummySnapshots = (() => {
  const now = new Date();
  const snaps = {};
  // base values that grow slightly each month
  const base = [
    { cash: { c1: 18200, c2: 4500 }, inv: { i1: { invested: 12000, current: 13100 }, i2: { invested: 8000, current: 8400 }, i3: { invested: 5000, current: 4800 } }, cpf: { oaCash: 24000, cpfisInvested: 4000, cpfisCurrent: 4200, sa: 14000, ma: 9500 }, income: 6800, exp: { Housing:1800,Food:520,Transport:280,Entertainment:120,Health:60,Shopping:200,Insurance:180,Subscriptions:45,Travel:0,["Family/Parents"]:300,Others:80 } },
    { cash: { c1: 19400, c2: 4500 }, inv: { i1: { invested: 12000, current: 13450 }, i2: { invested: 8000, current: 8550 }, i3: { invested: 5500, current: 5200 } }, cpf: { oaCash: 24900, cpfisInvested: 4000, cpfisCurrent: 4450, sa: 14600, ma: 9800 }, income: 6800, exp: { Housing:1800,Food:480,Transport:280,Entertainment:200,Health:0,Shopping:350,Insurance:180,Subscriptions:45,Travel:0,["Family/Parents"]:300,Others:120 } },
    { cash: { c1: 20100, c2: 5200 }, inv: { i1: { invested: 12000, current: 13800 }, i2: { invested: 8000, current: 8700 }, i3: { invested: 5500, current: 5600 } }, cpf: { oaCash: 26000, cpfisInvested: 4000, cpfisCurrent: 4600, sa: 15200, ma: 10100 }, income: 6800, exp: { Housing:1800,Food:560,Transport:280,Entertainment:80,Health:200,Shopping:150,Insurance:180,Subscriptions:45,Travel:800,["Family/Parents"]:300,Others:60 } },
    { cash: { c1: 21500, c2: 5200 }, inv: { i1: { invested: 13000, current: 14500 }, i2: { invested: 9000, current: 9400 }, i3: { invested: 5500, current: 5900 } }, cpf: { oaCash: 27000, cpfisInvested: 4000, cpfisCurrent: 4900, sa: 15800, ma: 10400 }, income: 8200, exp: { Housing:1800,Food:500,Transport:280,Entertainment:160,Health:0,Shopping:280,Insurance:180,Subscriptions:45,Travel:0,["Family/Parents"]:300,Others:90 } },
    { cash: { c1: 23200, c2: 5200 }, inv: { i1: { invested: 13000, current: 15100 }, i2: { invested: 9000, current: 9800 }, i3: { invested: 5500, current: 6200 } }, cpf: { oaCash: 28200, cpfisInvested: 4000, cpfisCurrent: 5000, sa: 16400, ma: 10700 }, income: 6800, exp: { Housing:1800,Food:490,Transport:280,Entertainment:140,Health:80,Shopping:220,Insurance:180,Subscriptions:45,Travel:0,["Family/Parents"]:300,Others:70 } },
    { cash: { c1: 24800, c2: 6000 }, inv: { i1: { invested: 14000, current: 16200 }, i2: { invested: 10000, current: 10600 }, i3: { invested: 5500, current: 6500 } }, cpf: { oaCash: 29400, cpfisInvested: 4000, cpfisCurrent: 5200, sa: 17000, ma: 11000 }, income: 6800, exp: { Housing:1800,Food:510,Transport:280,Entertainment:190,Health:0,Shopping:310,Insurance:180,Subscriptions:45,Travel:600,["Family/Parents"]:300,Others:95 } },
  ];
  base.forEach((d, idx) => {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!snaps || Object.keys(snaps).length === 0) { snaps = dummySnapshots; await save(KEYS.snapshots, snaps); }
      setProfile(p);
      setSources(s);
      setSnapshots(snaps);
      setLoaded(true);
          ...
          <div>
            ...
            {[...(sources?.cash||[])].sort((a,b)=>a.label.localeCompare(b.label)).map(c => (
              <div key={c.id} style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                <span style={{ fontSize:14, color:"#555", width:140, flexShrink:0 }}>{c.label}</span>
            ...
            {(sources?.investments||[]).map(inv => (
              <div key={inv.id} style={{ marginBottom:12 }}>
                <div style={{ fontSize:14, color:"#555", marginBottom:6, fontWeight:500 }}>{inv.label}</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  <div>
                    <label className="label">Amount Invested (Cost)</label>
